...
function replaceText(word, replacement, text) {
  ...
  return text.replace(rgx, replacement);
}
...
function extractTextInParentheses(str) {
  ...
  const rgx = /\((.*?)\)/g;
  return Array.from(str.matchAll(rgx), (e) => e[1]);
}
...
function countEmails(str) {
  ...
  const rgx = /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b/g;
  return Array.from(str.match((rgx), (e)=>e[0])).length
}
...
function replaceWordOccurrences(str, word, newWord) {
  ...
  const rgx = new RegExp(word, 'g');
  return str.replace(rgx, newWord);
}
...
function checkRegexMethods(regex) {
  ...
  let arr = [`dotAll`, `multiline`, `sticky`]
  let mtds = []
